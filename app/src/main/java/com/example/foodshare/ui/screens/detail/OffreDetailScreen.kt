...
fun OffreDetailScreen(
	...
	Column(
		...
		Row(verticalAlignment = Alignment.CenterVertically) {
			...
			Text(text = "Détail de l'offre", color = WhiteText, fontSize = 22.sp, fontWeight = FontWeight.Bold)
		}
			...
			offer != null -> {
				Card(colors = CardDefaults.cardColors(containerColor = DarkSurface), shape = RoundedCornerShape(20.dp), modifier = Modifier.fillMaxWidth()) {
					Column(modifier = Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
						Text(text = offer?.title ?: "Offre", color = WhiteText, fontSize = 22.sp, fontWeight = FontWeight.Bold)
						...
						Button(onClick = { /* reserve later */ }, colors = ButtonDefaults.buttonColors(containerColor = OrangeAccent)) {
							Text(text = "RÉSERVER")
						}
					}
				}
