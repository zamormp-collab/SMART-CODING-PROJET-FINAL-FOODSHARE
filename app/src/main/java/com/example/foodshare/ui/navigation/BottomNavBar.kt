...
fun BottomNavBar(navController: NavHostController, currentRoute: String?) {
    NavigationBar(
        ...
        // Offres
        NavigationBarItem(
            selected = currentRoute == Screen.Offres.route,
            onClick = {
                navController.navigate(Screen.Offres.route) {
                    popUpTo(Screen.Home.route) { inclusive = false }
                    launchSingleTop = true
                }
            },
            icon = { Icon(Icons.Default.ShoppingCart, contentDescription = "Offres") }
        )
        ...
        NavigationBarItem(
            ...
            icon = { Icon(Icons.Default.Favorite, contentDescription = "Réservations") }
        )
